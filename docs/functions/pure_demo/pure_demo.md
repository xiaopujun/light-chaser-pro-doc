...
> 纯演示模式并非完整的LCP服务，因此他的渲染效果存在一定的局限性。
> - 纯演示模式只能渲染单个项目，不支持服务项目之间的嵌套引用
> - 纯演示模式不支持将数据库作为数据源的场景
> - 纯演示模式不支持将公共API作为数据源的场景
> - 纯演示模式不支持大屏引用组件

> 购买纯演示服务[请点击](https://lcpdesigner.cn/shop)
...
演示模式的后端使用SQLite作为数据库，不需要单独安装数据库服务。
